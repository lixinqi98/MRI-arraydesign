...
%% N9
% Mona: add evenly distributed 6cm loops between two 4.5cm loops in back
% head
N9 = 16;
r6 = 0.03;
x1 = r6*cos((0:359)*pi/180);
z1 = r6*sin((0:359)*pi/180);
rr04 = rr1+(rr-rr1)*1;
anglezz = 165;
anglezzz = 135;
angle4z0 = [[0.9 1 1.01 1.01 -0.9 -1 -1.01 -1.01].*anglezz*pi/180, [0.9 1 1.01 1.01 -0.9 -1 -1.01 -1.01].*anglezzz*pi/180];
angle4x0 = repmat([55 30 5 0 55 30 5 0].*pi/180, [1,2]);
...
for jr = N1+N2+N3+N4+N5+N6+N7+N8+1:N1+N2+N3+N4+N5+N6+N7+N8+N9
    index = jr - (N1+N2+N3+N4+N5+N6+N7+N8);
    coordi1 = [x1; y1; z1];
...
%% N10 
% Mona: add evenly distributed 6cm loops to forehead and size(corespondong
% to N2)
% N10 = 7;
% rr01 = (rr+rr1)/2;
% angle1y0  = [0 linspace(30,180-30,7-2) 0]*pi/180;
% anglezz = 65;
% angle1z0 = [1.15 1.12 0.98 0.9 0.98 1.12 -1.15].*anglezz*pi/180;
% anglexx = -5;
% angle1x0 = [0 1 2 3 2 1 0].*anglexx*pi/180;
% 
% x1 = r6*cos((0:359)*pi/180);
% z1 = r6*sin((0:359)*pi/180);
% 
% for jr = N1+N2+N3+N4+N5+N6+N7+N8+N9+1:N1+N2+N3+N4+N5+N6+N7+N8+N9+N10
%     
%     y1 = rr01*ones(1,size(x1,2));
%     y2 = y1;
%     
%     index = jr - (N1+N2+N3+N4+N5+N6+N7+N8+N9);
% 
%     coordi1 = [x1; y1; z1];
% 
%     angle1z = angle1z0(index);
...
for jr = N1+N2+N3+N4+N5+N6+N7+N8+N9+N10+1:N1+N2+N3+N4+N5+N6+N7+N8+N9+N10+N11
    
    angle0001 = 66;
    angle0002 = 90;
    angle0003 = 110;
    
    angle01 = [angle0001 angle0001 angle0001 angle0001 ...
        angle0003 angle0003 angle0003 angle0003...
        -angle0001 -angle0001 -angle0001 -angle0001...
        -angle0003 -angle0003 -angle0003 -angle0003]*pi/180;
    angle02 = 20/180*pi;
    ...
    c = [0 0 1]'.*0.05;
    zoffset0 = repmat(c,1,size(coordi1,2));
    z_factor = [-1.5 -0.8 0 0];

    index = jr - (N1+N2+N3+N4+N5+N6+N7+N8+N9+N10);
    rot2 = [1 0 0 ; 0 cos(angle02) -sin(angle02); 0 sin(angle02) cos(angle02)];
...
N12 = 15;
rr01 = (rr+rr1)/2;
angle1y0  = repmat([40 linspace(60,180-60,5-2) -40]*pi/180, [1 3]);
anglezz1 = 50;
anglezz2 = 80;
anglezz3 = 100;
angle1z0 = [[1.1 0.95 0.9 0.95 -1.1].*anglezz1*pi/180, [1.1 0.95 0.9 0.95 -1.1].*anglezz2*pi/180, [1.1 0.95 0.9 0.95 -1.1].*anglezz3*pi/180];
anglexx = -5;
angle1x0 = repmat([1 2 3 2 1].*anglexx*pi/180, [1 3]);

x1 = r6*cos((0:359)*pi/180);
z1 = r6*sin((0:359)*pi/180);

for jr = N1+N2+N3+N4+N5+N6+N7+N8+N9+N10+N11+1:N1+N2+N3+N4+N5+N6+N7+N8+N9+N10+N11+N12
    
    y1 = rr01*ones(1,size(x1,2));
    y2 = y1;
    
    index = jr - (N1+N2+N3+N4+N5+N6+N7+N8+N9+N10+N11);
...
%% N13 
% Mona: add 6cm loops on the top of head
% to N2)
N13 = 4;
rr01 = (rr+rr1)/2;
angle1y0  = [[linspace(60,180-60,5-2)]*pi/180, pi/2];
anglezz1 = 110;
angle1z0 = [[0.95 0.9 0.95].*anglezz1*pi/180, 0.9*120*pi/180];
anglexx = -5;
angle1x0 = [0 0 0 0].*anglexx*pi/180;

x1 = r6*cos((0:359)*pi/180);
z1 = r6*sin((0:359)*pi/180);

for jr = N1+N2+N3+N4+N5+N6+N7+N8+N9+N10+N11+N12+1:N1+N2+N3+N4+N5+N6+N7+N8+N9+N10+N11+N12+N13
    
    y1 = rr01*ones(1,size(x1,2));
    y2 = y1;
    
    index = jr - (N1+N2+N3+N4+N5+N6+N7+N8+N9+N10+N11+N12);
